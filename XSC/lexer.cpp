    ...
    Token GetNextToken ()
    {
        ...
        switch ( iCurrLexState )
        {
            ...
            case LEX_STATE_IDENT:
                ...
                if ( strcasecmp ( g_CurrLexerState.pstrCurrLexeme, "var" ) == 0 )
                    TokenType = TOKEN_TYPE_RSRVD_VAR;

                // true

                if ( strcasecmp ( g_CurrLexerState.pstrCurrLexeme, "true" ) == 0 )
                    TokenType = TOKEN_TYPE_RSRVD_TRUE;

                // false

                if ( strcasecmp ( g_CurrLexerState.pstrCurrLexeme, "false" ) == 0 )
                    TokenType = TOKEN_TYPE_RSRVD_FALSE;

                // if

                if ( strcasecmp ( g_CurrLexerState.pstrCurrLexeme, "if" ) == 0 )
                    TokenType = TOKEN_TYPE_RSRVD_IF;

                // else

                if ( strcasecmp ( g_CurrLexerState.pstrCurrLexeme, "else" ) == 0 )
                    TokenType = TOKEN_TYPE_RSRVD_ELSE;

                // break

                if ( strcasecmp ( g_CurrLexerState.pstrCurrLexeme, "break" ) == 0 )
                    TokenType = TOKEN_TYPE_RSRVD_BREAK;

                // continue

                if ( strcasecmp ( g_CurrLexerState.pstrCurrLexeme, "continue" ) == 0 )
                    TokenType = TOKEN_TYPE_RSRVD_CONTINUE;

                // for

                if ( strcasecmp ( g_CurrLexerState.pstrCurrLexeme, "for" ) == 0 )
                    TokenType = TOKEN_TYPE_RSRVD_FOR;

                // while

                if ( strcasecmp ( g_CurrLexerState.pstrCurrLexeme, "while" ) == 0 )
                    TokenType = TOKEN_TYPE_RSRVD_WHILE;

                // func

                if ( strcasecmp ( g_CurrLexerState.pstrCurrLexeme, "func" ) == 0 )
                    TokenType = TOKEN_TYPE_RSRVD_FUNC;

                // return

                if ( strcasecmp ( g_CurrLexerState.pstrCurrLexeme, "return" ) == 0 )
                    TokenType = TOKEN_TYPE_RSRVD_RETURN;

                // host

                if ( strcasecmp ( g_CurrLexerState.pstrCurrLexeme, "host" ) == 0 )
