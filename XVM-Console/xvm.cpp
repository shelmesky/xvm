...
// ---- Data Structures -----------------------------------------------------------------------
	...
	// ---- Scripts ---------------------------------------------------------------------------
		...
		typedef struct _Script							// Encapsulates a full script
		{
            ...
            iInstrStream InstrStream;                    // The instruction stream
            RuntimeStack Stack;                         // The runtime stack
            iFuncTable FuncTable;                        // The function table
			iHostAPICallTable HostAPICallTable;			// The host API call table
		}
			Script;
			...
			switch ( iOpcode )
			{
                ...
                case INSTR_CALLHOST:
                {
                    ...
                    for (iHostAPIFuncIndex = 0; iHostAPIFuncIndex < MAX_HOST_API_SIZE; ++ iHostAPIFuncIndex )
                    {
                        ...
                        if ( strcasecmp ( pstrFuncName, pstrCurrHostAPIFunc ) == 0 )
                        {
    ...
    */
    ...
    int GetFuncIndexByName ( int iThreadIndex, char * pstrName )
    {
        ...
        for ( int iFuncIndex = 0; iFuncIndex < g_Scripts [ iThreadIndex ].FuncTable.iSize; ++ iFuncIndex )
        {
            ...
            if ( strcasecmp ( pstrName, g_Scripts [ iThreadIndex ].FuncTable.pFuncs [ iFuncIndex ].pstrName ) == 0 )
                return iFuncIndex;
        }
